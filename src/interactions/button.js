    ...
    async execute(client, config, interaction) {
        ...
        if (type === 'accept') {
            try {
                ...
                await user.send(` تم قبولك في تقديم الادارة في  ${config.APPLY[field].NAME}`);
            } catch (error) {
                ...
                const role2ID = config.APPLY[field].DATA.find((i) => i.name === name)?.role2;

                   
                if (role2ID) {
                    try {
                        const guildUser = await interaction.guild.members.fetch(userId);
                        await guildUser.roles.add(role2ID);
                    } catch (error) {
